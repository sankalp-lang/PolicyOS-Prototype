/* Dashboard — shared home (centered Ask-Tara hero + actions + KPIs + activity).
    ...
    return `<div class="page">${hero}
      ...
      <div class="card mt-16"><div class="card__head">${App.icon('plug')}<h3>Connected sources</h3><div class="spacer"></div><span class="muted" style="font-size:12px">Permission-faithful retrieval across all sources</span></div>
        <div class="card__body"><div class="row wrap gap-8">
          ${DB.connectors.filter(c => c.status === 'connected').map(c => `<span class="conn-chip">${App.conn.logo(c.id, 16)} ${App.esc(c.name)} <span class="muted" style="font-weight:450">· ${App.esc(c.count)}</span></span>`).join('')}
        </div></div></div>
    </div>`;
  }
});
