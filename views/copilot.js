...
App.registerView('copilot', {
  ...
  render(ctx) {
    ...
    const u = ctx.user;
    const suggestions = [
      { q:"Who's in the Engineering team?", ic:'users', tag:'HRMS' },
      { q:'Who is working on PolicyOS?', ic:'branch', tag:'Jira' },
      { q:'Is Sankalp in office today?', ic:'clock', tag:'HRMS' },
      { q:"What's the leave policy?", ic:'shield', tag:'Policy' },
      { q:'What is Abhishek Chaudhary working on?', ic:'branch', tag:'Jira' },
      (u.role==='user'
        ? { q:"What's the travel & expense policy?", ic:'briefcase', tag:'Policy' }
        : { q:'Personal loan eligibility criteria?', ic:'file', tag:'Policy' })
    ];
    const empty = `<div style="max-width:560px;margin:8vh auto 0;text-align:center">
        ...
        <p class="muted" style="margin-top:8px;font-size:14px">Tara pulls from HRMS, Jira, Notion and your policies — and only ever returns what your role is permitted to see.</p>
    ...
    return `<div class="page" style="max-width:820px;display:flex;flex-direction:column;height:calc(100vh - 150px)">
      <div class="row gap-8" style="margin-bottom:8px;flex-wrap:wrap">
        <span class="src-chip hrms">${App.icon('users')} HRMS</span>
        <span class="src-chip jira">${App.icon('branch')} Jira</span>
        <span class="src-chip" >${App.icon('book')} Notion</span>
        <span class="src-chip policy">${App.icon('shield')} Policies</span>
      ...
      <div class="chat-foot" style="border:1px solid var(--line);border-radius:16px;background:var(--surface)">
        <div class="chat-inputwrap" style="border:none">
          <textarea id="copilotInput" rows="1" placeholder="Ask about people, work in progress, or a policy…" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();App.copilot.ask();}"></textarea>
